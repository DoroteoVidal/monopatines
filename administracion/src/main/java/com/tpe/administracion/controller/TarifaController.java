...
public class TarifaController {
    ...
    public ResponseEntity<?> guardarTarifa(@RequestBody Tarifa t) {
        try{
            return ResponseEntity.status(HttpStatus.CREATED).body(service.guardar(t));
        }catch(Exception e) {
    ...
    @PreAuthorize("hasAnyAuthority(\"" + AuthorityConstant.ADMIN + "\" )")
    public ResponseEntity<?> definirTarifaExtra(@PathVariable Long id, @RequestBody Double tarifaExtra) {
        try{
            return ResponseEntity.status(HttpStatus.CREATED).body(service.definirExtra(id, tarifaExtra));
        }catch(Exception e) {
